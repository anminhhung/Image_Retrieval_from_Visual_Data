...
class RandomIdentitySampler(Sampler):
    ...
    def __init__(self, trainCSVPath, mini_batch_size, num_instances):
        self.dataframe = pd.read_csv(trainCSVPath)
    ...
    def sample_list(self):
        ...
        # for single gpu
        num_iter_per_epoch = float(len(avai_labels)/self.num_pids_per_gpu)
        ...
        pid_count = 0
        while True:
            indices = torch.randperm(len(avai_labels))
            for i in indices:
...
class ImageUniformSampler(Sampler):
    """
    Randomly sample N identities, then for each identity,
    randomly sample K instances, therefore batch size is N*K.
    Args:
    ...
    def sample_list(self):
        ...
        batch_indices = []
        while len(avai_labels) >= self.num_pids_per_batch:
            selected_pids = np.random.choice(avai_labels, self.num_pids_per_batch, replace=False).tolist()
            for label in selected_pids:
                ...
                avai_idxs = batch_idxs_dict[label]
                for _ in range(self.num_instances):
                    batch_indices.append(avai_idxs.pop(0))

                if len(avai_idxs) < self.num_instances: avai_labels.remove(label)
