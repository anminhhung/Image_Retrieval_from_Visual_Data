...
class Net(nn.Module):
    def __init__(self, cfg, margins):
        super(Net, self).__init__()

        self.cfg = cfg
        self.n_classes = self.cfg.num_of_classes
        self.backbone = timm.create_model(cfg.backbone, 
                                          pretrained=cfg.pretrained, 
                                          num_classes=0, 
                                          global_pool="", 
                                          in_chans=self.cfg.input_dim,features_only = True)
...
class Net_Lightning(LightningModule):
    def __init__(self, cfg, dataset) -> None:
        ...
        tmp = np.sqrt(1 / np.sqrt(dataset.df['landmark_id'].value_counts().sort_index().values))
        ...
        if self.cfg.loss == 'adaptive_arcface':
            self.loss_fn = ArcFaceLossAdaptiveMargin(
                self.margins, self.n_classes, cfg.arcface_s)
        elif self.cfg.loss == 'arcface':
            self.loss_fn = ArcFaceLoss(cfg.arcface_s, cfg.arcface_m)
        else:
            pass
    ...
    def train_dataloader(self):
        ...
        return DataLoader(self.dataset, batch_size=self.cfg.batch_size, num_workers=self.cfg.num_workers,
